...
class Portfolio(bases.BasePortfolio):
    """Represents a Portfolio record"""
    ...
    def create_partial(
        name: str,
        include: Optional[Iterable['types.PortfolioKeys']] = None,
        exclude: Optional[Iterable['types.PortfolioKeys']] = None,
        required: Optional[Iterable['types.PortfolioKeys']] = None,
        optional: Optional[Iterable['types.PortfolioKeys']] = None,
        relations: Optional[Mapping['types.PortfolioRelationalFieldKeys', str]] = None,
        ...
        fields: Dict['types.PortfolioKeys', PartialModelField] = OrderedDict()

        try:
            if include:
                for field in include:
                    fields[field] = _Portfolio_fields[field].copy()
            elif exclude:
                for field in exclude:
                    if field not in _Portfolio_fields:
                        raise KeyError(field)

                fields = {
                    key: data.copy()
                    for key, data in _Portfolio_fields.items()
            ...
            if exclude_relational_fields:
                fields = {
                    ...
                    if key not in _Portfolio_relational_fields
                }

            if relations:
                for field, type_ in relations.items():
                    if field not in _Portfolio_relational_fields:
                        raise errors.UnknownRelationalFieldError('Portfolio', field)
            ...
            raise ValueError(
                f'{exc.args[0]} is not a valid Portfolio / {name} field.'
            ) from None
...
_Portfolio_relational_fields: Set[str] = {
        'assets',
        'transactions',
    }
_Portfolio_fields: Dict['types.PortfolioKeys', PartialModelField] = OrderedDict(
    [
        ('id', {
            'name': 'id',
            'is_list': False,
